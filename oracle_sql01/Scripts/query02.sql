 ...
 * 			Record Set 에는 Record 가 0개 이상 포함되어 있다. (데이터가 없을 수도 있다.) 
 */

SELECT * FROM EMPLOYEES;

/*
 * SELECT 구문에는 FROM절,  WHERE 절, ORDER BY 절 등이 추가로 사용 된다. 
 * FROM 절에는 조회할 테이블명을 작성하게 된다. 
 * WHERE 절에는 FROM 절에 명시한 테이블에서 데이터를 조회하기 위한 조건을 작성하게 된다.
 * ORDER BY 절에는 조회 결과에 대한 정렬 기준을 작성하게 된다. 
 * SELECT 절에는 FROM 절에 명시한 테이블의 컬럼을 작성하게 된다. 이 때 작성된 컬d럼의 
 * 데이터들만 Record Set 으로 반환이 된다. (* 모든 컬럼 / SELECT 필요한 컬럼 명 필요 컬럼 반환)
 */
SELECT  EMPLOYEE_ID, FIRST_NAME, LAST_NAME FROM EMPLOYEES;

/*
 * WHERE 조건절 : 사용자가 지정한 조건에 해당하는 Record Set 만 반환 
 */
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME FROM EMPLOYEES WHERE LAST_NAME = 'Grant';

/*
 * ORDER BY  : 정렬 / 정렬 조건 컬럼을 제시하면 그 컬럼을 중심으로 정렬
 * 					 + DESC; 내림차순 
 */
...
/*
* SELECT 절에 사용하는 컬럼명이  Record Set 으로 반환 될 때 그래도 출력되지만 
...
/*
 * SELECT 절에 컬럼명과 리터럴 값(' ')을 혼합하여  Record Set 으로 반환되는 
 * 결과값을 변경 할 수 있다. 
 */
SELECT 'NO. ' || EMPLOYEE_ID AS "사원 번호 "
 ...
 /*
  * SELECT 절에 컬럼과 컬럼을 결합하여 하나의 컬럼으로 만들어 Record Set 을 
  * 반환할 수 있다.
  */
...
/*
* 숫자 데이터가 저장된 컬럼에는 사칙연산을 수행한 결과로 Record Set 을 
* 반환할 수 있다. 
*/
SELECT FIRST_NAME
			, LAST_NAME
 ...
 * 		AND : 모든 결과가 true 일 때 ture 반환
 * 		OR : 하나의 결과가 ture 이면 true 반환
 * 		NOT : 반환된 결과에 대한 부정 
*/
SELECT FIRST_NAME
			, LAST_NAME
