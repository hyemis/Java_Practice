...
SELECT  CASE WHEN BEFORE_HOUR = 0 THEN
				...
				END AS X
 FROM (SELECT FLOOR(SYSDATE  -  TO_DATE('20210213',  'YYYYMMDD')) AS BEFORE_DAY
			,  FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('2021021',  'YYYYMMDD'))) AS BEFORE_MONTH
			,  FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('2021021',  'YYYYMMDD')) / 12) AS BEFORE_YEAR
			,  FLOOR(TO_NUMBER(SYSDATE - TO_DATE('2021021',  'YYYYMMDD')) * 24) AS BEFORE_HOUR
			, FLOOR(TO_NUMBER (SYSDATE  -  TO_DATE('2021021',  'YYYYMMDD')) * 24 * 60) AS BEFORE_MINUTE
			FROM DUAL);
