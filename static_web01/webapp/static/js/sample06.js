 ...
 * .before(추가할엘리먼트)  : 형제 앞에 추가
 * .after(추가할엘리먼트) : 형제 뒤에 추가
 */
...
function addRow(element) {
    /* 
        ...
        조회수는 기본 0으로 설정 한다.  
    */
   var totalRow = tbdoy.children.length;
   var tbdoy = document.getElementById("table").lastElementChild;
    var now = new Date();
...
function createTalbeRow(columnCount){
...
function  validCheck(form) {
     ...
     * 패스워드 확인 입력 양식 옆에 "다시 입력하시오." 라는 메세지가 나오도록
     * 합니다. username 에서는 6자리 이상의 아이디가 아닌 경우 패스워드와 동일하게 
     * 아이디 입력 양식 옆에 "6자리 이상 입력하십시오." 라는 메시지가 나오도록 합니다.
     */
